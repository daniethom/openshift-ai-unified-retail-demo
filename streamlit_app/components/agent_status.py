...
class AgentStatusDisplay:
    ...
    def render_agent_card(self, agent_name: str, status: Dict[str, Any]):
        ...
        with st.container():
            ...
            with col1:
                st.metric("Queries", metrics.get("queries_processed", 0), delta=None)

            with col2:
                success_rate = metrics.get("success_rate", 1.0)
                st.metric("Success Rate", f"{success_rate * 100:.1f}%", delta=None)

            with col3:
                avg_time = metrics.get("avg_response_time", 0)
                st.metric("Avg Time", f"{avg_time:.2f}s", delta=None)
    ...
    def render_agent_grid(self, agent_statuses: Dict[str, Dict[str, Any]]):
        ...
        specialist_agents = {
            k: v for k, v in agent_statuses.items() if k != "HomeAgent"
        }
    ...
    def render_system_metrics(self, agent_statuses: Dict[str, Dict[str, Any]]):
        ...
        active_agents = sum(
            1 for status in agent_statuses.values() if status.get("status") == "active"
        )
        ...
        with col2:
            st.metric(
                "Active Agents", f"{active_agents}/{len(agent_statuses)}", delta=None
            )
        ...
        with col4:
            st.metric("Collaborations", total_collaborations, delta=None)
            ...
            marker=dict(
                ...
                color=[
                    self.status_colors.get(
                        agent_statuses[agent].get("status", "offline"), "#9E9E9E"
                    )
                    for agent in agents
                ],
    ...
    def render_system_health(self, agent_statuses: Dict[str, Dict[str, Any]]):
        ...
        active_agents = sum(
            1 for s in agent_statuses.values() if s.get("status") == "active"
        )
        error_agents = sum(
            1 for s in agent_statuses.values() if s.get("status") == "error"
        )
        ...
        with col1:
            st.metric(
                "System Health Score", f"{health_score:.0f}%", delta=f"{health_status}"
            )
...
class AgentMetricsTracker:
        ...
        for snapshot in st.session_state.metrics_history:
            ...
            if agent_name:
                ...
                if agent_name in snapshot["metrics"]:
                    ...
                    data.append(
                        {"Timestamp": timestamp, "Value": value, "Agent": agent_name}
                    )
            else:
                # All agents
                for agent, metrics in snapshot["metrics"].items():
                    ...
                    data.append(
                        {"Timestamp": timestamp, "Value": value, "Agent": agent}
                    )
    ...
    def render_trends(self):
        ...
        if len(st.session_state.metrics_history) < 2:
            st.info(
                "Insufficient data for trends. Metrics will appear as the system processes queries."
            )
            return
