...
from .knowledge_base import DocumentStore, MilvusKnowledgeBase
...
class RAGRetriever:
    ...
    async def retrieve(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(enhanced_results)} documents for query: {query[:50]}..."
            )
        ...
        for result in results:
            ...
            if doc_type == "product":
                enhanced_result["source"] = (
                    f"Product Catalog - {metadata.get('brand', 'Unknown')}"
                )
                enhanced_result["display_title"] = metadata.get(
                    "name", "Unknown Product"
                )
            elif doc_type == "trend":
                enhanced_result["source"] = (
                    f"Trend Analysis - {metadata.get('season', 'Unknown')}"
                )
                enhanced_result["display_title"] = metadata.get(
                    "title", "Unknown Trend"
                )
                ...
                enhanced_result["source"] = (
                    f"Market Research - {metadata.get('source', 'Unknown')}"
                )
                enhanced_result["display_title"] = metadata.get(
                ...
                enhanced_result["display_title"] = (
                    f"Document {result.get('id', 'Unknown')}"
                )

            # Format content preview
            content = result.get("content", "")
            enhanced_result["preview"] = (
        ...
        try:
            ...
            if doc_type == "product":
                ...
                for doc in documents:
                    processed_doc = {
                        ...
                        "metadata": {"type": doc_type, **doc.get("metadata", {})},
                    }
            ...
            for doc in processed:
                self.document_store.save_document(
                    doc_id=doc["id"], content=doc, doc_type=doc_type
                )
    ...
    async def update_document(
        self, doc_id: str, content: Dict[str, Any], doc_type: str = "custom"
    ) -> bool:
        """
    ...
    async def delete_documents(
        self, doc_ids: List[str], doc_type: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def initialize_from_data_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"imported": import_counts, "indexed": total_stats}
...
class HybridRetriever(RAGRetriever):
    ...
    async def retrieve(
        ...
        keyword_results = await self._keyword_search(
            query=query, top_k=top_k * 2, doc_type=doc_type
        )
    ...
    async def _keyword_search(
        self, query: str, top_k: int, doc_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for doc_meta in all_docs:
            ...
            if doc:
                ...
                if score > 0:
                    results.append(
                        {**content, "score": score, "search_type": "keyword"}
                    )
