...
async def invoke_llm(request: InvokeRequest):
    if not client:
        raise HTTPException(
            status_code=500,
            detail="LLM client not initialized. Check API key and base URL.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error communicating with LLM API: {e}"
        )
...
async def health_check():
    ...
    if client:
        ...
        raise HTTPException(
            status_code=503, detail="Service Unavailable: LLM client not configured"
        )
