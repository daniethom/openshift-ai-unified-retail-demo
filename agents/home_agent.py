...
class HomeAgent(BaseAgent):
    ...
    def __init__(
        self, mcp_servers: Dict[str, Any], agent_registry: Dict[str, BaseAgent]
    ):
        ...
        try:
            ...
            return {
                ...
                "metadata": {
                    ...
                    "agents_used": analysis.get("required_agents", []),
    ...
    def _determine_intent(self, query: str, keywords: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "scope": (
                "broad"
                if any(s in query_lower for s in ["overall", "all", "entire"])
                else "specific"
            ),
                ...
                if keyword in pattern_data["keywords"]:
                    ...
                    if intent["question_type"] == "analytical":
                        required_agents.update(
                            pattern_data.get("supporting_agents", [])
                        )
    ...
    def _assess_complexity(
        self, required_agents: List[str], intent: Dict[str, Any]
    ) -> QueryComplexity:
        """Assess query complexity"""
        num_agents = len(required_agents)
        is_analytical = intent["question_type"] == "analytical"

        if num_agents <= 1 and not is_analytical:
            return QueryComplexity.SIMPLE
        if num_agents <= 3 or (num_agents == 1 and is_analytical):
            return QueryComplexity.MODERATE
        if num_agents <= 4 and is_analytical:
            return QueryComplexity.COMPLEX
        return QueryComplexity.ADVANCED

    def _select_strategy(
        self, complexity: QueryComplexity, required_agents: List[str]
    ) -> str:
    ...
    def _identify_data_needs(
        self, intent: Dict[str, Any], keywords: List[str]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "customer_specific": any(
                k in keywords for k in ["customer", "loyalty", "personalize"]
            ),
        }

    def _enhance_context(
        self, context: Dict[str, Any], intent: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_moderate_query(
        ...
        if strategy == "parallel":
            results = await self._execute_parallel(
                query, analysis["required_agents"], analysis["context_enhanced"]
            )
        else:
        ...
        primary_agent = self._identify_primary_agent(
            analysis["required_agents"], analysis["keywords"]
        )
        return {
    ...
    async def _handle_complex_query(
        ...
        lead_agent = self._identify_lead_agent(
            analysis["required_agents"], analysis["intent"], analysis["keywords"]
        )
        results = await self._execute_hierarchical(
            query, lead_agent, analysis["required_agents"], analysis["context_enhanced"]
        )
        ...
        return {
            ...
            "primary_result": results.get(lead_agent, {}),
        }
    ...
    async def _handle_advanced_query(
        ...
        for agent_name in remaining_agents:
            ...
            if self._should_engage_agent(agent_name, results, analysis):
                agent_result = await self.agent_registry[agent_name].process_query(
                    query, context_enhanced
                )
        ...
        return {
            ...
            "insights": self._generate_strategic_insights(results, analysis),
        }

    async def _execute_parallel(
        self, query: str, agent_names: List[str], context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_sequential(
        ...
        for agent_name in agent_names:
            ...
            if agent:
                ...
                try:
                    results[agent_name] = await agent.process_query(
    ...
    async def _execute_hierarchical(
        ...
        support_results = await self._execute_parallel(
            query, support_agents, support_context
        )
        ...
        for agent in agents:
            for pattern_data in self.routing_patterns.values():
                if pattern_data["primary_agent"] == agent:
                    score = sum(
                        1 for keyword in keywords if keyword in pattern_data["keywords"]
                    )
                    agent_scores[agent] += score
        return max(agent_scores, key=agent_scores.get) if agent_scores else agents[0]

    def _identify_lead_agent(
        self, agents: List[str], intent: Dict[str, Any], keywords: List[str]
    ) -> str:
        if intent["question_type"] == "analytical":
            if "TrendAgent" in agents:
                return "TrendAgent"
            if "PricingAgent" in agents:
                return "PricingAgent"
    ...
    def _extract_support_requirements(self, lead_result: Dict[str, Any]) -> List[str]:
        ...
        return [
            req
            for req in ["inventory_levels", "customer_insights", "pricing_analysis"]
            if f"need_{req}" in str(lead_result)
        ]

    def _should_engage_agent(
        self, agent_name: str, current_results: Dict[str, Any], analysis: Dict[str, Any]
    ) -> bool:
        if agent_name in analysis.get("required_agents", []):
            return True
        return any(
            f"need_{agent_name.lower()}" in str(result)
            for result in current_results.values()
        )

    def _synthesize_multi_agent_results(
        self, results: Dict[str, Any]
    ) -> Dict[str, Any]:
        # Simplified for demonstration
        return {
            "key_findings": [
                res.get("primary_insight", "N/A")
    ...
    def _validate_results(
        self, synthesis: Dict[str, Any], intent: Dict[str, Any]
    ) -> Dict[str, Any]:
        # Simplified for demonstration
        gaps = []
        if intent["requires_action"] and not any(
            "recommend" in str(f) for f in synthesis.get("key_findings", [])
        ):
            gaps.append("Action recommendations missing")
        return {
            "intent_satisfied": not gaps,
            "completeness": 0.9,
            "confidence": 0.85,
            "gaps": gaps,
        }
    ...
    def _generate_cross_agent_insights(
        ...
        if "InventoryAgent" in results and "PricingAgent" in results:
            insights.append(
                {
                    "type": "inventory_pricing_alignment",
                    "insight": "Opportunity for dynamic pricing.",
                }
            )
        if "CustomerAgent" in results and "TrendAgent" in results:
            insights.append(
                {
                    "type": "customer_trend_alignment",
                    "insight": "Customer preferences align with trends.",
                }
            )
        return insights
    ...
    def _generate_strategic_insights(
        ...
        insights = []
        if all(
            a in results for a in ["PricingAgent", "InventoryAgent", "CustomerAgent"]
        ):
            insights.append(
                {
                    "type": "revenue_optimization",
                    "insight": "Potential for integrated revenue optimization.",
                }
            )
        return insights
    ...
    def _get_system_load(self) -> str:
        active_queries = sum(
            1 for agent in self.agent_registry.values() if agent.status == "busy"
        )
        if active_queries == 0:
            return "low"
        if active_queries <= len(self.agent_registry) / 2:
            return "medium"
        return "high"
    ...
    def _insights_tool(
        self, time_period: str = "24h", focus_areas: List[str] = None
    ) -> Dict[str, Any]:
    ...
    async def generate_system_insights(
        self, time_period: str = "24h", focus_areas: List[str] = None
    ) -> Dict[str, Any]:
        logger.info(f"Generating system insights for {time_period}...")
        # Dummy implementation
        return {
            "insights": "System performance is optimal.",
