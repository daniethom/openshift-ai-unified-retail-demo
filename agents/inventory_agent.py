...
class InventoryAgent(BaseAgent):
    ...
    def get_tools(self) -> List[Any]:
        ...
        return [self._check_stock_tool, self._forecast_tool, self._optimize_tool]
    ...
    def _classify_query(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["optimize", "improve", "reduce cost"]):
            ...
            word in query_lower for word in ["reorder", "replenish", "order more"]
        ):
            return "reorder"
        elif any(
    ...
    async def _handle_stock_check(
        ...
        stock_data = await self._get_stock_levels(
            product_id, location, include_warehouse
        )
    ...
    async def _handle_forecast(
        ...
        # Generate forecast
        forecast = self._generate_forecast(historical_data, period, include_seasonality)
    ...
    async def _handle_reorder(
        ...
        return {
            ...
            "urgent_items": len(
                [r for r in order_recommendations if r["urgency"] == "high"]
            ),
    ...
    async def _get_stock_levels(
        self, product_id: str, location: str, include_warehouse: bool
    ) -> Dict[str, Any]:
    ...
    def _calculate_availability(self, stock_data: Dict[str, Any]) -> str:
        ...
        total_available = sum(
            loc_data.get("available", 0) for loc_data in stock_data.values()
        )
    ...
    def _check_reorder_status(
        self, stock_data: Dict[str, Any], product_id: str
    ) -> Dict[str, Any]:
        """Check if product needs reordering"""
        total_stock = sum(
            loc_data.get("on_hand", 0) for loc_data in stock_data.values()
        )
        ...
        lead_time_days = 7
        safety_stock = (
            avg_daily_demand * self.inventory_params["safety_stock_multiplier"]
        )
    ...
    def _generate_stock_recommendations(
        self, stock_data: Dict[str, Any], product_id: str, availability: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_reorder_items(
        self, location: str, urgent_only: bool
    ) -> List[Dict[str, Any]]:
    ...
    def _group_by_supplier(
        self, order_recommendations: List[Dict[str, Any]]
    ...
    def _get_general_recommendations(
        self, inventory_health: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _check_stock_tool(
        self, product_id: str, location: str = "all"
    ) -> Dict[str, Any]:
    ...
    def _forecast_tool(
        self, product_id: str, period: str = "30_days"
    ) -> Dict[str, Any]:
    ...
    def _analyze_inventory_performance(
        ...
        return {
            ...
            "stockout_rate": (
                stockout_count / total_products if total_products > 0 else 0
            ),
            "overstock_rate": (
                overstock_count / total_products if total_products > 0 else 0
            ),
            "turnover_rate": self._calculate_turnover_rate(inventory_data),
            "service_level": (
                1 - (stockout_count / total_products) if total_products > 0 else 1
            ),
        }
    ...
    def _calculate_turnover_rate(self, inventory_data: List[Dict[str, Any]]) -> float:
        ...
        avg_inventory = sum(item["current_stock"] for item in inventory_data) / len(
            inventory_data
        )
    ...
    async def _get_historical_data(self, product_id: str) -> List[Dict[str, Any]]:
        ...
        for i in range(365):
            ...
            daily_sales = int(
                base_demand * seasonal_factor * trend_factor * random_factor
            )
    ...
    def _generate_forecast(
        ...
        days_to_forecast = {"7_days": 7, "30_days": 30, "90_days": 90}.get(period, 30)
    ...
    def _identify_demand_factors(
        self, historical_data: List[Dict[str, Any]], product_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_forecast_recommendations(
        self, forecast: List[Dict[str, Any]], product_id: str
    ) -> List[Dict[str, Any]]:
