...
def agent_pair():
    ...
    """
    agent_a = MockConcreteAgent(
        name="AgentA", role="TesterA", goal="Test A", backstory="..."
    )
    agent_b = MockConcreteAgent(
        name="AgentB", role="TesterB", goal="Test B", backstory="..."
    )
...
def test_agent_initialization():
    ...
    agent = MockConcreteAgent(
        name="TestAgent", role="Tester", goal="Test Goal", backstory="..."
    )
...
def test_update_metrics():
    ...
    # Arrange
    agent = MockConcreteAgent(
        name="MetricAgent", role="Tester", goal="Test Goal", backstory="..."
    )
...
async def test_send_and_receive_message_query(agent_pair):
    ...
    response_message = await agent_a.send_message(
        recipient="AgentB", content={"query": query_content}
    )
...
async def test_receive_message_error_handling(agent_pair):
    ...
    error_response = await agent_a.send_message(
        recipient="AgentB", content={"query": error_query}
    )
...
def test_message_history_limit():
    ...
    # Arrange
    agent = MockConcreteAgent(
        name="HistoryAgent", role="Tester", goal="Test", backstory="..."
    )
    ...
    assert (
        agent.message_history[0].content == "Msg 5"
    )  # Check the first message is the correct one
