...
def pricing_agent(mock_data_store, mock_mcp_servers):
    ...
    agent._get_current_pricing = AsyncMock(
        return_value={"product_id": "P1", "current_price": 100, "cost": 60}
    )
    agent._get_market_data = AsyncMock(
        return_value={"market_average": 110, "price_range": {"min": 90, "max": 120}}
    )
    agent._get_demand_data = AsyncMock(
        return_value={"current_demand": "stable", "price_sensitivity": -1.5}
    )
    return agent
